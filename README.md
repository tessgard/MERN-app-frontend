...
**Maintainability** refers to how easy it will be to update the application in the futue as new requirements arise or changes need to be made. If an application is well designed it should be easily updatable, as most likely it will be updated by a different person than the original author. Maintenance accounts for just under 70% of the cost of software, a lot more than the cost of the initial design and build.

**Correctness** is used when referring to whether the software meets all the requirements original stated and defined at the start of the software design process. As some software products are very complex, this can be difficult to assess and test 100%. 

**Reusability** refers to the ability for the code to be reused to build other software. This means that developpers can produce much morecomplex software products in shorter period of time by using code that is already written. This has been of huge importance in developping modern technologies and innovative software products.

**Reliability** talks to the frequency of errors. In some cases such as air traffic control or heart monitors, reliability becomes the most important quality measure. In the case of event ticket vendors, reliability in the event of high traffic is especially important.

**Portability** refers to the ability for the software to be ported to a different computer configuration. Good software should have a relatively long lifespan and we could expect hardware technologies to change and update multiple times throughout a software's life. It's important for this to not become a barrier.

**Efficiency** talks to how the software uses resources to run, in the most efficient way possible. An example within our application is the use of lazy loading for our image gallery to ensure that unless the user is scrolling down the page, the software is not wasting energy on loading the images.
